...
"""
--- test_ms_filter_vcf.py ---

Function for testing if ms flagged variants can be correctly filtered
...
# Tests if a filtered VCF can be be created from a flagged VCF
def test_ms_filtered_vcf_output(clean_workspace_fixture):
    ...
    for sample in ms_sample:
        ...
        files_to_copy = [f"{sample}_ms_merged_flagged.vcf.gz",
                         f"{sample}_ms_merged_flagged.vcf.gz.tbi"]
    ...
    for sample in ms_sample:
        filtered_vcf_path = Path("tmp") / sample / f"{sample}_ms_filtered_variants.vcf.gz"

        # Check if filtered VCF exists
        assert filtered_vcf_path.exists(), f"ms_filtered_variants.vcf.gz not found: {filtered_vcf_path}"

        # Check that filtered VCF is not empty
        assert filtered_vcf_path.stat().st_size > 0, f"ms_filtered_variants.vcf.gz is empty: {filtered_vcf_path}"
