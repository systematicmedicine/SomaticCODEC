...
2. Tests that all component metrics that use regexes, are tested by this test
...
def build_test_cases():
    ...
    if merged["value_pattern"].isnull().any():
        ...
        raise ValueError(
            "Missing regex pattern for the following metrics:\n"
            f"{missing[['Metric', 'File_path']]}"
        )
...
def test_regex_matches_expected(metric, file_path, pattern, expected):
    ...
    try:
        ...
        raise RuntimeError(
            f"Error calling R function for {metric} in {file_path}: {e}"
        )

    assert str(result) == str(expected), (
        f"Mismatch for {metric} in {file_path}: "
        f"expected '{expected}', got '{result}'"
    )


def test_all_regexes_tested():
    # Filter config for metrics with file_path ending in '.txt' or '.csv'
    txt_metrics = config_df[
        config_df["file_pattern"].str.endswith((".txt", ".csv"), na=False)
    ]

    # Get the set of metrics present in expected values CSV
    expected_metrics = set(expected_df["Metric"])

    # Get the set of metrics with txt/csv files in config
    txt_metrics_set = set(txt_metrics["Name"])

    # Find any metrics with txt/csv files missing from expected values
    missing_metrics = txt_metrics_set - expected_metrics

    assert not missing_metrics, (
        "The following metrics use regexes but are not included in regex test:\n"
        f"{missing_metrics}"
    )