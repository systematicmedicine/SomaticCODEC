...
# Write header
echo "time,disk_used_GB,disk_avail_GB,disk_IOPS,mem_used_GB,mem_avail_GB,cpu_load,cpu_avail" > "$LOG_FILE"
(
    while true; do
        ...
        disk_IOPS=$(iostat -dx 1 1 | awk 'NR>6 {sum += $2 + $8} END {print sum}')
        ...
        echo "$now,$disk_used_GB,$disk_avail_GB,$disk_IOPS,$mem_used_GB,$mem_avail_GB,$cpu_load,$cpu_avail" >> "$LOG_FILE"
